...
const createPaymentIn = async (req, res) => {
    try {
        ...
        const newPayment = new PaymentIn({
            InvoiceNo,
            PartyName,
            PaymentType,
            Date,
        ...
        res.status(201).json({ message:MESSAGES.SUCCESS.PAYMENTIN_CREATED, companyCode });
...
const updatePaymentIn = async (req, res) => {
    try {
        ...
        const oldPayment = await PaymentIn.findById(id);
        ...
        // Update main PaymentIn record
        const updatedPayment = await PaymentIn.findByIdAndUpdate(id, updateFields, { new: true });
        ...
        res.status(200).json({ message:MESSAGES.SUCCESS.PAYMENTIN_UPDATED  , updatedPayment });
...
const FetchPaymentIn = async (req, res) => {
    try {
        const companyCode = req.user["companyCode"];
        const { PaymentIn } = await getCompanyModels(companyCode);
...
const deletePaymentIn = async (req, res) => {
    try {
        ...
        const deletedPayment = await PaymentIn.findByIdAndDelete(id);
        if (!deletedPayment) {
            return res.status(404).json({ message: MESSAGES.ERROR.RECORD_NOT_FOUND });
        }

        res.status(200).json({ message: MESSAGES.SUCCESS.PAYMENTIN_DELETED, deletedPayment });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
...
export { createPaymentIn, FetchPaymentIn, updatePaymentIn, deletePaymentIn };
